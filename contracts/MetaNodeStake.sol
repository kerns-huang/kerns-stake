...
contract MetaNodeStake is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(IERC20 _metaNode,uint256 _startBlock,uint256 _endBlock,uint256 _metaNodePerBlock) public initializer {
        ...
        _grantRole(DEFAULT_ADMIN_ROLE, msg.ender);
     ...
     */
    function addPool(address stTokenAddress, uint256 poolWeight, uint256 minDepositAmount, uint256 unstackLockedBlocks) public {
    ...
    // 计算用户等待领取的奖励
    function pendingReward(uint256 pid, address userAddress) public view returns (uint256 reward){
        Pool storage pool = pools[pid];
        User storage user = userInfos[pid][userAddress];
        // 计算用户的质押金额能拿到多少个meta代币
        bool success = false;
